...
void ConstantExpressionResult::my_handle_tree(ASTNode const *t)
{
    BEGIN_CONTEXT_CATCH;
    Expression const *expr = t->ifExpressionC();
    if (!expr) {
        VXCOV;
        return;
    }
    ...
    bool valueUsed = true;
    if (Statement const *parentStmt = expr->parent->ifStatementC()) {
        if (parentStmt->isS_expr()) {
            VXCOV;
            valueUsed = false;
        } else {
            VXCOV;
    ...
    ASTSWITCH(expr) {
    ASTCASE(E_cast, cast) {
        // In Javascript, we expect an implicit cast to a boolean when we
        // see conditionals that are not themselves a comparison.
        // However, in CLike, we see an implicit != 0, for boolification.
        // This emulates that feature, so we don't have to change the checker
        // much to make it do work
        if(cast->ckind == CK_IMPLICIT && cast->type->isscalar_type_t() &&
            cast->type->ifscalar_type_tC()->scalar_kind() == types::scalars::KIND_BOOL) {
            E_intLit zero_tmp(cast->loc, cast->type, 0);
            E_binary* b = new (*curTraversal->get_arena()) E_binary(
                cast->loc,
                cast->type,
                cast->expr,
                BIN_DYNAMIC_NOTEQUAL,
                &zero_tmp,
                PI_IMPLICIT);
            b->parent = cast->parent;
            my_handle_tree(b);
        }
    }
    ASTCASE(E_binary, biny) {
        ...
        if(isUnsignedCompareZero(biny)) {
            // Pretend we reported an error, to avoid reporting an
            // error in a composed expression, e.g. i || u >= 0
            rememberReportingError();
            return;
        }

        Expression const *e1 = biny->e1;
        Expression const *e2 = biny->e2;
        switch (biny->op) {
        case BIN_DYNAMIC_EQUAL:
            VXCOV; goto GEQ_case;
        case BIN_DYNAMIC_NOTEQUAL:
        ...
        case BIN_GREATEREQ:
            ...
            VXCOV;
GEQ_case:
            // Don't use Evals_to, as "eval" should take care of the
            // interesting parts, and we definitely don't want to
            // strip casts.
            bitset_t E1 = eval(e1);
            bitset_t E2 = eval(e2);
            bitset_t RESULT = bitset_t::nonIntegralBottom();
            ...
            default:
                xfailure("Operator changed?");
            }

            if (e1->isE_javascriptTypeof()) {
                error =
                    checkJavaScriptTypeofComparison(expr,
                                                    e1->asE_javascriptTypeofC(),
                                                    e2,
                                                    biny->op,
                                                    curTraversal);
            } else if (e2->isE_javascriptTypeof()) {
                error =
                    checkJavaScriptTypeofComparison(expr,
                                                    e2->asE_javascriptTypeofC(),
                                                    e1,
                                                    biny->op,
                                                    curTraversal);
            } else
            if ((value_really_not_known(E1, e1) ||
                 value_really_not_known(E2, e2)) &&
                RESULT.valueKnown()) {

                Expression const *parentExpr = expr->ifExpressionParent();
                E_binary const *biny1;
                // Here we're mostly looking for situations where a non-Boolean
                // subexpression is possibly inadvertently being used as a
                // Boolean, which would mean it was under a compiler-generated
                // Bool-ification node, but for the case we're looking to *not*
                // report -- where an assignment of some sort is Bool-ified for
                // use in a conditional -- we want to include explicit Bool-
                // ification as well.  Consequently, 'boolify' is really
                // 'implicitly_boolify', but that's unwieldy.
                bool boolify = isCompilerGeneratedBoolification(expr);

                if (isBoolification(expr) &&
                    // 'expr' is the ==/!= node, so its parent might be
                    // && or || and its first child would then be the actual
                    // operation
                    parentExpr != NULL &&
                    (parentExpr->isBinary(BIN_AND) ||
                     parentExpr->isBinary(BIN_OR)) &&
                    e1->isE_assign()) {
                    VXCOV;
                    // "if (a && (b |= 1))"
                    rememberReportingError();
                } else if (boolify && e1->isUnary(UNY_BITNOT) &&
                           // is suggested change better than original?
                           !( !eval(static_cast<E_unary const *>(e1)->expr) ).
                                  valueKnown()) {
                    // "if (~_)" intended to be
                    // "if (!_)"
                    VXCOV;
                    error = createErrorWithEventAndContextAndElaboration(
                        "logical_vs_bitwise",
                        "logical_vs_bitwise",
                        MESSAGE(
                               E(expr) << " is always "
                               << (RESULT.knownTrue() ? "1/true" : "0/false")
                               << " regardless of the values of its operand."),
                        expr,
                        MESSAGE(
                           "Did you intend to use '!' rather than '~'?"));
                    // sup_assert(isCLike(curTraversal->getLang()));
                } else if (boolify && parentExpr && parentExpr->isE_cond() &&
                           (biny1 = e1->ifE_binaryC()) != NULL) {
                    // "_ | _ ? _ : _" intended to be
                    // "_ | (_ ? _ : _)"
                    string es = MESSAGE(
                        E(expr) << " is always "
                        << (RESULT.knownTrue() ? "true" : "false")
                        << " regardless of the values of its operands.");
                    string elaboration;
                    string elaboration2;
                    E_cond const *cond = parentExpr->ifE_condC();
                    bitset_t E2_redo = evalCond(cond, E2, eval(cond->th),
                                                eval(cond->el));
                    bitset_t RESULT_redo = redo(biny->op, E1, E2_redo);
                    if (checkRedo(E1, E2_redo, RESULT_redo)) {
                        VXCOV;
                        tag = "missing_parentheses";
                        category = "missing_parentheses";
                        elaboration = MESSAGE(
                           "Did you intend only " << E(biny1->e2)
                           << " to be the controlling expression for the "
                           << "ternary ('?:') expression?");
                        elaboration2 = EMESSAGE(
                            "If so, parentheses would be required "
                            "around the entire ternary expression to force "
                            "this interpretation.");
                        // sup_assert(isCLike(curTraversal->getLang()));
                    } else {
                        VXCOV;
                        tag = "result_independent_of_operands";
                        category = "result_independent_of_operands";
                    }
                    error = createErrorWithEventAndContextAndElaborations(
                        tag, category, es, expr, elaboration, elaboration2);
                } else if (boolify && (biny1 = e1->ifE_binaryC()) &&
                           (biny1->op == BIN_BITAND ||
                            biny1->op == BIN_BITOR) &&
                           (is_relop(biny1->e1) || is_relop(biny1->e2))) {
                    // "if ((_ & 2) & (_ == _))" intended to be
                    // "if ((_ & 2) /* != 0 */ && (_ == _))"
                    // TODO: either this logic here is wrong, or this case is
                    // being covered by some other pattern, since the examples
                    // above trigger defects, but not *this* particular defect
                    VCOV;
                    error = createErrorWithEventAndContextAndElaboration(
                        "logical_vs_bitwise",
                        "logical_vs_bitwise",
                    ...
                    sup_assert(isCLike(curTraversal->getLang()));
                           ...
                           biny1->op == BIN_BITOR) {
                    VXCOV;
                    // "if (_ | constant)" intended to be
                    // "if (_ & constant)"; this suggestion always appears
                    ...
                    // 'expr' is the implicit != node, so its parent might be
                    // ! and its expr would then be the actual operation
                    parentExpr != NULL && parentExpr->isUnary(UNY_NOT)) {
                    // if (!<some-non-bool-expr>)
                    VXCOV;
                    error = createErrorWithEventAndContext(
                        "result_independent_of_operands",
                        "result_independent_of_operands",
                        MESSAGE(
                               E(static_cast<E_unary const *>(parentExpr)->expr)
                               << " is always "
                               << (RESULT.knownTrue() ? "1/true" : "0/false")
                               << " regardless of the value of its operand."),
                        expr);
                    // Explicit booleans do not become comparison in Javascript
                    sup_assert(isCLike(curTraversal->getLang()));
                } else if (!boolify && is_relop(biny) && e1->isUnary(UNY_NOT)) {
                    // "if (!_ == _)" intended to be
                    // "if (!(_ == _))" or "if (_ != _)"
                    VXCOV;
                    event_stream &es = desc()
                        ...
                        << (RESULT.knownTrue() ? "true" : "false")
                        << " regardless of the values of its operands.";
                    // Evaluate !(_ == _), if that's not constant,
                    // they probably meant it.
                    bitset_t E1_redo = eval(
                        skipCompilerGeneratedBoolification(
                            static_cast<E_unary const *>(
                                e1)->expr)
                    );
                    // Attempt the comparison with the type of the
                    // operand that was negated (ideally we'd do the
                    // appropriate integer promotions but this'll have
                    // to do)
                    bitset_t RESULT_redo = redo(
                        invertRelopPolarity(biny->op),
                        E1_redo,
                        E2.castTo(E1_redo.typeParms)
                    );
                    if (checkRedo(E1_redo, E2, RESULT_redo)) {
                        VXCOV;
                        tag = "missing_parentheses";
                        ...
                        error = createErrorWithEventAndContextAndElaboration(
                            tag, category,
                            consume_event_stream(es),
                            expr,
                            consume_event_stream(desc()
                               << "Did you intend to either negate the entire "
                               << "comparison expression, in which case "
                               << "parentheses would be required around the "
                               << "entire comparison expression to force that "
                               << "interpretation, or negate the sense of the "
                               << "comparison (that is, use '"
                               << toStr(invertRelopPolarity(biny->op), curLang)
                               << "' rather than '" << toStr(biny->op, curLang) << "')?")
                            );
                    } else {
                        VXCOV;
                        tag = "result_independent_of_operands";
                        category = "result_independent_of_operands";
                        error = createErrorWithEventAndContext(tag, category,
                            consume_event_stream(es), expr);
                    }
                } else if ((!boolify || !e1->isE_assign() ||
                            e1->asE_assignC()->op != BIN_ASSIGN ||
                            condition.match(expr)) &&
                           !is_size_based_bounds_check_and_not_loop_condition(biny)) {
                    if (same_no_side_effect(e1, e2)) {
                        if (any_sobs_exclusions(expr, e1, e2, curTraversal)) {
                            VXCOV;
                        } else {
                            VXCOV;
                            error = createErrorWithEventAndContext(
                                ...
                                MESSAGE(
                                    ...
                                    << (RESULT.knownTrue() ? "true" : "false")
                                    << " regardless of the values of its operands "
                                    << "because those operands are "
                                    << (same_expr_memory(e1, e2) ? "identical" :
                    ...
                    // "boolVal = (intVar = 0);",
                    // "bool f() {return intVal = 0;}"
                    // "funcTakingBoolArg(intVal = 0);
                    // or size-based bound check
                    VXCOV;
                    rememberReportingError();
                }
            } else {
                // same-on-both-sides pertains to the code actually written, so
                // boil off anything compiler-generated
                e1 = skipCompilerGeneratedBoolification(e1);
                e2 = skipCompilerGeneratedBoolification(e2);
                if (same_no_side_effect(e1, e2) &&
                    biny->isImplicit == PI_EXPLICIT &&
                    // rule out "0==0", "1==1", "0!=0", "1!=1"
                    !((biny->op == BIN_EQUAL || biny->op == BIN_NOTEQUAL) &&
                      (is0OrFalse(e1) || is1OrTrue(e1))) &&
                    !any_sobs_exclusions(expr, e1, e2, curTraversal)) {
                    VXCOV;
                    error = createErrorWithEventAndContext(
                        "same_on_both_sides",
                        "same_on_both_sides",
        ...
        case BIN_OR:
            ...
            VXCOV;
OR_case:
            bool const is_oror = biny->op == BIN_OR;
            bool const is_andand = biny->op == BIN_AND;
            bitset_t E1 = eval(e1);
            bitset_t E2 = eval(e2);
            bitset_t RESULT = is_oror ? (E1 || E2) : (E1 && E2);
            // RESULT may have type "int".
            // Make sure "valueKnown()" checks it as a bool.
            RESULT = RESULT.castTo(itype_parms(/*bits*/ 1, /*is_unsigned*/ true));
            CDOUT (*expr << ", E1: " << E1 << ", E2: " << E2 << ", RESULT: " << RESULT);
            if ((value_really_not_known(E1, e1) ||
                 value_really_not_known(E2, e2)) &&
                RESULT.valueKnown()) {
                // By default, we don't report "if (A&B && _)" or
                // "if ((A&B) || _)" when 'A' and 'B' are constants.  This
                // is because there are frequently occurring idioms where
                // some sort of bitwise constant expression is used to do
                // code configuration.
                // We always exclude purely logical subexpressions composed
                // entirely of literals, on similar (but more certain)
                // grounds.
                //
                // yxing:
                // Since constant-folding works differntly for clang and edg, even
                // for c and cpp. Thus isXDueToLiteral difinitely can not work.
                //
                // According to the churn, let's make the rule simple:
                // we do not report by default if the RESULT is caused
                // by constants. Moreover, we treat pure false/0/true/1 as
                // intentional because that's how people disable some code
                // in the real world.
                bool is_false_e1 = false, is_false_e2 = false,
                     is_true_e1 = false, is_true_e2 = false;

                // see test-intentional
                // see jtest-intentional
                bool is_intentional = false;

                if (is_andand) {
                    is_false_e1 = E1.knownFalse() && isConstOrLiteral(e1);
                    is_false_e2 = E2.knownFalse() && isConstOrLiteral(e2);

                    is_intentional = (is0OrFalse(e1) || is0OrFalse(e2));

                    CDOUT ("false_e1: " << is_false_e1
                            << ", false_e2: " << is_false_e2
                            << ", intentional: " << is_intentional);
                } else if (is_oror) {
                    is_true_e1 = E1.knownTrue() && isConstOrLiteral(e1);
                    is_true_e2 = E2.knownTrue() && isConstOrLiteral(e2);

                    is_intentional = (is1OrTrue(e1) || is1OrTrue(e2));

                    CDOUT("true_e1: " << is_true_e1
                            << ", true_e2: " << is_true_e2
                            << ", intentional: " << is_intentional);
                }

                if (is_intentional) {
                    CDOUT ("Ignore: intentional ");
                    rememberReportingError();
                    return;
                }

                if ((error = checkBitwiseVsLogicalAndOr(expr, e1, e2, is_andand)) ==
                     NULL) {
                    bool is_const_operands =
                        (is_andand && (is_false_e1 || is_false_e2))
                         || (is_oror && (is_true_e1 || is_true_e2));

                    if (is_const_operands) {
                        CDOUT ("constant logical operands");
                        if(!reportConstantLogicalOperands()) {
                            CDOUT ("Ignore: constant ");
                            rememberReportingError();
                            return;
                        }
                        ...
                        if(!report_constant_logical_operands_in_macros
                                &&curTraversal->fn_trav().is_in_macro(expr)) {
                            CDOUT ("Ignore: constant in macro ");
                            rememberReportingError();
                            return;
                        }
                    }
                ...
                if (same_no_side_effect(e1, e2) &&
                    ...
                    !any_sobs_exclusions(expr, e1, e2, curTraversal)) {
                    VXCOV;
                    error = create_nopath_error(*curTraversal);
                    error->add_event("pointless_expression",
                                     desc() << "The expression " << E(expr)
                                     << " does not accomplish anything "
                                     << "because it evaluates to either "
                                     << "of its identical operands, "
                                     << E(e1) << ".",
                                     curTree);
                    error->set_main_event();
                    error->add_remediation_event("remediation",
                                                 desc() <<
                                                     ...
                                                     "different?",
                                                 curTree);
                    error->set_subcategory("same_on_both_sides");
                } else {
                    VXCOV;
                    error = checkBitwiseVsLogicalAndOr(expr, e1, e2, is_andand);
                    if (error == NULL && !parent_is_same(biny)) {
                        // TODO: checker option
                        bool allow_comparison_to_non_constant = false;
                        ComparandCollector comparand_collector(
                            biny, allow_comparison_to_non_constant);
                        error = comparand_collector.reportError(this,
                                                                curTraversal,
                                                                expr,
                                                                is_andand);
                    } else {
        ...
        case BIN_BITAND:
            VXCOV; goto BITOR_case;
        case BIN_BITXOR:
            VXCOV; goto BITOR_case;
        case BIN_BITOR:
            VXCOV;
BITOR_case:
            error = checkBitwise(expr, e1, e2, biny->op);
            break;

        case BIN_LSHIFT:
            VXCOV; goto RSHIFT_case;
        case BIN_URSHIFT:
            VXCOV; goto RSHIFT_case;
        case BIN_RSHIFT:
            {
            VXCOV;
RSHIFT_case:
            Expression const *realParent;
            // note: this hides the global 'condition'
            ConditionPattern condition(/*matchSwitch*/ false);
            bitset_t E1 = eval(e1);
            bitset_t E2 = eval(e2);
            Expression const *not_boolification;
            // TODO: Perhaps report cases where shift value is 0
            // in Java/JS where only the lowest 5 bits matter(32, 64 are all 0)
            if ((value_really_not_known(E1, e1) ||
                 value_really_not_known(E2, e2)) &&
                (not_boolification = skipEnclosingCompGenBoolification(expr)) &&
                (realParent = not_boolification->ifExpressionParent()) !=
                 NULL) {
                // TODO: perhaps handle URSHIFT with E1.castTo(E1.sizeInBytes(), false)
                bitset_t R = evalShift(biny->op, E1, E2, curTraversal->getLang());
                // At one point we used Evals_from(expr) here, which allows
                // pretty far-fetched things like "if (x, y<<z)" to be caught,
                // but that has the unwanted side effect of turning e into
                // _?e:_ or _?_:e so that it incorrectly matches 'condition'.
                // If it were worth the trouble we could write a variant of
                // Evals_from() that doesn't include ternaries, but that seems
                // like more trouble than it's worth.
                if (condition.match(not_boolification) &&
                    (E1.knownFalse() || (E1.knownTrue() && R.knownTrue()))) {
                    tag      = "result_independent_of_operands";
                    category = "result_independent_of_operands";
                    string es = MESSAGE(
                        "In " << E(expr) << ", shifting " << E(e1)
                        << " does not change its logical value (of "
                        << (E1.knownTrue() ? "1/true" : "0/false") << ").");
                    // sup_assert(isCLike(curTraversal->getLang()));

                    string elaboration;
                    if (E_binary const *binyParent=realParent->ifE_binaryC()) {
                        // The only binary operators with Boolean operands are
                        // '&&' and '||'.

                        // We could test these changes before suggesting them,
                        // but doing so is very complicated -- just look at
                        // checkBitwise().
                        if (binyParent->op == BIN_AND) {
                            VXCOV;
                            tag = "logical_vs_bitwise";
                            category = "logical_vs_bitwise";
                            elaboration = MESSAGE(
                               "Did you intend to use '&' rather "
                               << "than '&&'?");
                            goto elaboration_done;
                        } else {
                            VXCOV;
                            sup_assert(binyParent->op == BIN_OR);
                            tag = "logical_vs_bitwise";
                            category = "logical_vs_bitwise";
                            elaboration = MESSAGE(
                               "Did you intend to use '|' rather "
...
#if HAVE_TESTCASE_FOR_L_IN_PLACE_OR_R_SHIFT
                    if (biny->op == BIN_LSHIFT) {
                        bitset_t RESULT_redo = E1 >> E2;
                        if (!E1.knownFalse() &&
                            (!E1.knownTrue() || !RESULT_redo.knownTrue())) {
                            // suggested change is better than original
                            VCOV;
                            tag = "operator_confusion";
                            category = "operator_confusion";
                            elaboration = MESSAGE(
                                "Did you intend a right shift ('>>')?");
                            goto elaboration_done;
                        } else {
                            VCOV;
                        }
...
elaboration_done:
                    error = createErrorWithEventAndContextAndElaboration(
                        tag, category, es, expr, elaboration);
                } else {
                    VXCOV;
                }
                if (R.valueKnown() && error == NULL) {
                    VXCOV;
                    error = createErrorWithEventAndContext(
                        "result_independent_of_operands",
                        "result_independent_of_operands",
    ...
    ASTCASE(E_assign, asgn) {
        ...
        case BIN_ASSIGN:
            ...
            VXCOV;
#if REAL_SOON_NOW
// This won't work well enough until bitset_t::operator==() gets the sign- and
// zero-extension correct for differing size operands.  Since it was originally
// meant for use only in actual comparisons, where the operands have been cast
// to a common size, this wasn't originally an issue.
            scalar_type_t const *type = expr->type->as_scalar_p();
            if (type == NULL || !type->is_integral()) {
                break;
            }
            Expression const *src = asgn->src;
            if (E_intLit const *intLit = src->ifE_intLitC()) {
                if (intLit->original_expr) {
                    src = intLit->original_expr;
                }
            }
            if (E_cast const *cast = src->ifE_castC()) {
                if (cast->ckind == CK_IMPLICIT) {
                    src = cast->expr;
                }
            }
            bitset_t SRC = eval(src);
            Expression const *target = asgn->target;
            bitset_t TARGET = eval(target);
            cout << "SRC: " << SRC << ", TARGET: " << TARGET << endl;
            cout << "SRC == TARGET: " << (SRC == TARGET) << endl;
            if ((TARGET == SRC).knownFalse() &&
                // It is common to set all of the bits in a bitfield
                // variable by assigning -1/~0 of arbitrary width into it,
                // so don't complain about that.
                !(SRC.valueKnown() && SRC.signedValue() == -1)) {
                // Report the narrowest known width of 'src' so as to not
                // say silly things like "5 is 32 bits".
                int leftMostZero;
                for (leftMostZero = SRC.sizeInBits;
                     leftMostZero > 0 &&
                       (SRC.knownZero & (((bitype)1) << (leftMostZero - 1)));
                    --leftMostZero) {
                }
                int srcWidth = leftMostZero;

                // For most targets we don't know of any zero bits so this
                // simply yields sizeInBits, but for bitfields sizeInBits
                // is the "type size", not the "storage size", and the
                // extra "type" bits are all known to be zero so we can
                // infer the actual bitfield width.  This won't be necessary
                // if we explicitly store the "storage width".
                for (leftMostZero = TARGET.sizeInBits;
                     leftMostZero > 0 &&
                       (SRC.knownZero & (((bitype)1) << (leftMostZero - 1)));
                    --leftMostZero) {
                }
                int targetWidth = leftMostZero;
                event_stream &es = desc() << "The destination " << E(target);
                if (targetWidth < srcWidth) {
                    es << " (of width " << targetWidth << " bits)";
                }
                es << " cannot represent the value " << E(src);
                if (targetWidth < srcWidth) {
                    es << " (of width " << srcWidth << " bits)";
                }
                es << " being assigned to it";
                error = createErrorWithEvent("assignment_loses_information",
                                             consume_event_stream(es));
            }
#endif
            }
        ...
        case BIN_BITAND:
            VXCOV; goto BITXOR_case;
        case BIN_BITOR:
            VXCOV; goto BITXOR_case;
        case BIN_BITXOR:
            VXCOV;
BITXOR_case:
            error = checkBitwise(expr, asgn->target, asgn->src, asgn->op);
            break;
        case BIN_MINUS: //-=, need to check if it is like y-=y, which expands to constant result
            VCOV;  error = checkBitwise(expr, asgn->target, asgn->src, asgn->op);
            break;
        default:
            VXCOV; // +=, etc.
            ;
    ...
    // E_unary never happens because the operand of ! will have been converted
    // to bool with an implicit != node and any defect will have been reported
    // there

    ASTCASE(E_cond, cond) {
        Expression const* e1 = NULL;
        Expression const* e2 = NULL;
        bool isandand = true;

        // In JavaScript, && and || will return one of the operands,
        // but we'd still like to report some defects on them, so
        ...
        if(cond->condKind == CEK_JAVASCRIPT_AND) {
            ...
            // Handle only Javascript cases here
            break;
        }
        ...
        if(e1 != NULL && e2 != NULL) {
            ...
            if (same_no_side_effect(e1, e2) &&
                ...
                error = checkBitwiseVsLogicalAndOr(cond, e1, e2, isandand);
            }
        }
...
// The pattern "_ ? MACRO1 : MACRO2", where MACRO1 and MACRO2 happen to have
// the same value, results in too many FPs from this particular check.  We would
// need a way to determine if e2 or e3 are macro expansions (and suppress in
// that case).  We could suppress if there are *any* macros involved, but such
// constructs frequently involve macros (and/or are entirely within macros) and
// thus too much would be lost.
        bitset_t COND = eval(cond->cond);
        bitset_t TH = eval(cond->th);
        bitset_t EL = eval(cond->el);
        bitset_t RESULT = evalCond(COND, TH, EL);
        if ((!COND.valueKnown() || !TH.valueKnown() || !EL.valueKnown()) &&
            RESULT.valueKnown() && !(COND.knownTrue() && TH.valueKnown()) &&
            !(COND.knownFalse() && EL.valueKnown())) {
            scalar_type_t const *type = expr->type->as_scalar_p();
            if (type && type->scalar_kind() == types::scalars::KIND_BOOL) {
                error = createErrorWithEvent("result_independent_of_operands",
                    MESSAGE(E(expr) << " is always "
                           << (RESULT.knownTrue() ? "true" : "false")
                            << " regardless of the value of its operand"));
            } else {
                error = createErrorWithEvent("result_independent_of_operands",
                    MESSAGE(E(expr) << " is always "
                           << hexOnIt(RESULT.value())
                            << " regardless of the value of its operand"));
            }
    ...
    // TODO: This currently applies only to C/C++, but could be extended
    // to Java/C#.  In that case, checker properties would need to be
    // copied to those domains.  See bug 53234.
    ASTCASE(E_funCall, fun_call) {
        if (fun_call->get_called_function() != NULL &&
            (str_equal(fun_call->get_called_function()->get_id(), "strcmp") ||
             str_equal(fun_call->get_called_function()->get_id(), "strncmp"))) {
            if (fun_call->args->size() >= 2) {
                if (Same(fun_call->getArg(0)).
                    match(fun_call->getArg(1))) {
                    error = createErrorWithEvent("pointless_string_compare",
                                                 "pointless_string_compare",
                        MESSAGE(*expr << " is always 0 because "
                                      << *fun_call->getArg(0)
    ...
    if (error) {
        VXCOV;
        error->set_extra_tree(expr, curTraversal->getLang());
        commit_error(error, *curTraversal);
    } else {
        VXCOV;
    }
    END_CONTEXT_CATCH("While analyzing " << *t);
}


void ConstantExpressionResult::get_dependencies(vector<string> &dependencies)
{
    VXCOV;
    // none
}
...
NamableSM *create_constant_expression_result()
{
    VXCOV;
    return create_checker_nofpp(new ConstantExpressionResult);
}
